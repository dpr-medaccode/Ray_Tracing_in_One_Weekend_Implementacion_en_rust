...
use crate::{
    ...
    Material::{Dielectrico::Dielectrico, Difuso_lambertiano::Difuso_lambertiano, Metal::Metal},
};

fn main() {
    // Lista de objetos
    let mut mundo = Lista_golpeable::new();

    // Suelo
    let material_suelo: Arc<dyn Material::Material> =
        Arc::new(Difuso_lambertiano::new(Vec3::Vec3::new(0.5, 0.5, 0.5)));
            ...
            if (center - Vec3::Vec3::new(4.0, 0.2, 0.0)).logitud() > 0.9 {
                if choose_mat < 0.8 {
                    ...
                    let material: Arc<dyn Material::Material> =
                        Arc::new(Difuso_lambertiano::new(albedo));
    ...
    mundo.push(Arc::new(Esfera::Esfera::new(Vec3::Vec3::new(0.0, 1.0, 0.0), 1.0, material1)));

    let material2: Arc<dyn Material::Material> = Arc::new(Difuso_lambertiano::new(Vec3::Vec3::new(0.4, 0.2, 0.1)));
    mundo.push(Arc::new(Esfera::Esfera::new(Vec3::Vec3::new(-4.0, 1.0, 0.0), 1.0, material2)));

    let material3: Arc<dyn Material::Material> = Arc::new(Metal::new(Vec3::Vec3::new(0.7, 0.6, 0.5), 0.0));
    mundo.push(Arc::new(Esfera::Esfera::new(Vec3::Vec3::new(4.0, 1.0, 0.0), 1.0, material3)));
